...
public interface TutorControllerOpenAPI {
    ...
    @Operation(summary = "Listar todos os tutores", description = "Retorna uma lista com todos os tutores cadastrados no sistema.")
    @ApiResponse(responseCode = "200", description = "Tutores listados com sucesso")
    ResponseEntity<List<TutorResponseDTO>> buscarTodos();

    @Operation(summary = "Cadastrar um novo tutor", description = "Cadastra um tutor na plataforma. O e-mail deve ser único.")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Excluir um tutor", description = "Exclui um tutor do sistema. Retorna erro se o tutor possuir histórico de adoções.")
    @ApiResponses(value = {
